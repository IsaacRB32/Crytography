...
int main() {
    ...
    int len_mensaje = (int) strlen(mensaje);
    
    unsigned char *ciphertext = malloc((size_t)len_mensaje + 1);
    if (ciphertext == NULL) {
    ...
    for (int i = 0; i < len_mensaje; i++) {
        ...
        unsigned char salida = cifrar_bloque(iv, k0, k1, k2, k3, S, P);

        ciphertext[i + 1] = (unsigned char)mensaje[i] ^ salida;
